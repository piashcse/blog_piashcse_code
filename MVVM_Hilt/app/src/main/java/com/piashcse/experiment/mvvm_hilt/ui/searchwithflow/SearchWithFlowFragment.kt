...
class SearchWithFlowFragment : Fragment() {
    ...
    private fun apiCall(since: String) = searchFlowViewModel.getRepositoryList(since).observe(viewLifecycleOwner) {
        when (it.status) {
            Status.LOADING -> {
                errorLog("Loading..")
            }
    ...
    private fun makeFlow(since: String) = searchFlowViewModel.makeFlow(since).observe(viewLifecycleOwner) {
        when (it.status) {
            Status.LOADING -> {
                errorLog("Loading..")
            }
